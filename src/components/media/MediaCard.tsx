  ...
  return (
    <>
      <article className="media-card group">
        ...
        <Link href={detailHref} className="focusable block truncate font-semibold text-white" data-focusable="true">{movie.title}</Link>
        <span>{movie.year || "Year unknown"}{movie.genre ? ` · ${movie.genre}` : ""}</span>
      </article>

      {menuOpen ? <div className="netflix-sheet-layer" role="presentation" onClick={() => setMenuOpen(false)}>
        <section className="netflix-action-sheet" role="dialog" aria-modal="true" aria-label={`Options for ${movie.title}`} onClick={(event) => event.stopPropagation()}>
          <header className="netflix-sheet-header">
            <h2>{movie.title}</h2>
            <button type="button" className="netflix-sheet-close" aria-label="Close" onClick={() => setMenuOpen(false)}>×</button>
          </header>
