...
export default function MediaToolsPage() {
  ...
  async function refreshJobs() {
    ...
    const result = await response.json() as { jobs?: ConversionJob[]; paused?: boolean; policyReason?: string | null };
    if (response.ok) { setJobs(result.jobs || []); setPaused(Boolean(result.paused)); setPolicyReason(result.policyReason || null); }
  }
  ...
  async function queueAll() {
    ...
    try {
      ...
      const result = await response.json() as { jobs?: ConversionJob[]; policyReason?: string | null; error?: string };
      if (!response.ok) throw new Error(result.error || "Could not queue conversions.");
      setJobs(result.jobs || []); setPolicyReason(result.policyReason || null);
  ...
  return <main className="admin-shell">
    ...
    {!paused && policyReason ? <div className="state-card conversion-paused"><strong>Automatically paused</strong><small>{policyReason}. It will resume automatically when policy allows.</small></div> : null}
