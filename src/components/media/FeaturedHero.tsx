...
function featuredSelection(movies: Movie[]) {
  ...
  return [...movies]
    .filter((movie) => movie.mediaType === "movie")
    .sort((left, right) => score(left.id, bucket) - score(right.id, bucket))
    .slice(0, 8);
}

function score(id: string, seed: number) {
  let value = seed | 0;
...
export function FeaturedHero({ movies }: { movies: Movie[] }) {
  ...
  useEffect(() => {
    if (featured.length < 2 || trailerOpen || paused || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    const timer = window.setInterval(() => setIndex((value) => (value + 1) % featured.length), ROTATE_MS);
    return () => window.clearInterval(timer);
  }, [featured.length, trailerOpen, paused]);

  function move(direction: number) {
    if (!featured.length) return;
    setTrailerOpen(false);
    setIndex((value) => (value + direction + featured.length) % featured.length);
  }
  ...
  return <section
    ...
    onTouchEnd={(event) => { const end = event.changedTouches[0]?.clientX; if (touchStart.current !== null && end !== undefined && Math.abs(end - touchStart.current) > 45) move(end < touchStart.current ? 1 : -1); touchStart.current = null; }}
  >
    <div key={`${current.id}-art`} className="hero-backdrop" style={artwork ? { backgroundImage: `url(${artwork})` } : undefined} />
    ...
    {!trailerOpen ? <div key={`${current.id}-copy`} className="hero-content hero-copy"><p className="eyebrow">FEATURED MOVIE</p><h1>{current.title}</h1><div className="hero-metadata"><span>{current.year || "Year unknown"}</span>{current.genres.slice(0, 2).map((genre) => <span key={genre}>{genre}</span>)}{current.rating !== null ? <span>★ {current.rating.toFixed(1)}</span> : null}{current.runtimeMinutes ? <span>{current.runtimeMinutes} min</span> : null}</div><p className="hero-overview">{current.overview || "Ready to stream from your private collection."}</p><div className="hero-actions"><Link href={`/tv/watch/${current.id}`} className="primary-button cinematic-play focusable" data-focusable="true">▶ Play Movie</Link>{current.trailerYouTubeId ? <button className="secondary-button cinematic-button focusable" data-focusable="true" onClick={() => setTrailerOpen(true)}>▷ Watch Trailer</button> : <Link href={`/tv/movie/${current.id}`} className="secondary-button cinematic-button focusable" data-focusable="true">More Info</Link>}</div></div> : null}
