...
async function processJob(job: ConversionJob) {
  ...
  const preferredMode: ConversionMode = sourceProbe.conversionMode || "transcode";
  const createOutput = async (mode: ConversionMode) => {
    await fs.rm(temp, { force: true });
  ...
  try {
    await createOutput(preferredMode);
  } catch (error) {
    if (preferredMode === "transcode") throw error;
    // Some streams report compatible codecs but cannot be safely copied into MP4.
    // Retrying with a full encode preserves the automatic, no-data-loss workflow.
    await createOutput("transcode");
  }
...
export async function enqueueConversion(relativePath: string, start = true) {
  ...
  const job: ConversionJob = { id: createHash("sha256").update(`${inside}:${now}`).digest("hex").slice(0, 16), source: inside, output, status: "queued", mode: probe.conversionMode || "transcode", progress: 0, durationSeconds: probe.durationSeconds, error: null, createdAt: now, updatedAt: now };
