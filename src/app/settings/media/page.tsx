...
export default function MediaToolsPage() {
  ...
  async function refreshJobs() {
    ...
    const result = await response.json() as { jobs?: ConversionJob[]; paused?: boolean; policyReason?: string | null; overrideActive?: boolean };
    if (response.ok) { setJobs(result.jobs || []); setPaused(Boolean(result.paused)); setPolicyReason(result.policyReason || null); setOverrideActive(Boolean(result.overrideActive)); }
  }
  ...
  async function queueAll() {
    ...
    try {
      const response = await fetch("/api/media/conversions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scan: true, runNow: true }) });
      const result = await response.json() as { jobs?: ConversionJob[]; policyReason?: string | null; overrideActive?: boolean; error?: string };
      if (!response.ok) throw new Error(result.error || "Could not queue conversions.");
      setJobs(result.jobs || []); setPolicyReason(result.policyReason || null); setOverrideActive(Boolean(result.overrideActive)); setPaused(false);
  ...
  return <main className="admin-shell">
    <header className="admin-header"><div><Link href="/settings">← System Status</Link><p className="eyebrow">MEDIA HEALTH</p><h1>Compatibility & Conversion</h1></div><div className="conversion-controls"><button className="secondary-button" disabled={pausing} onClick={() => void togglePause()}>{pausing ? "Updating…" : paused ? "▶ Resume overnight schedule" : "Ⅱ Pause conversions"}</button><button className="primary-button" disabled={queueing || loading} onClick={() => void queueAll()}>{queueing ? "Scanning…" : `Convert now (${incompatible})`}</button></div></header>
    ...
    {!paused && overrideActive ? <div className="state-card conversion-override"><strong>Convert-now override active</strong><small>The overnight schedule is temporarily bypassed until this queue finishes. Active streaming will still pause conversion automatically.</small></div> : null}
    {!paused && !overrideActive && policyReason ? <div className="state-card conversion-paused"><strong>Automatically paused</strong><small>{policyReason}. Select Convert now to override the overnight schedule, or it will resume automatically during the next window.</small></div> : null}
