...
import { listOfflineDownloads, offlineStorageEstimate, removeOfflineDownload, verifyOfflineDownload, type OfflineDownload } from "@/lib/offline/client";
...
export default function OfflineLibraryPage() {
  ...
  const [playing, setPlaying] = useState<OfflineDownload | null>(null);
  const [playbackError, setPlaybackError] = useState("");
  const refresh = useCallback(async () => {
    const saved = await listOfflineDownloads();
    const checked = await Promise.all(saved.map(async (item) => item.status === "ready" && !await verifyOfflineDownload(item) ? { ...item, status: "failed" as const, error: "This saved copy is incomplete. Remove it and download it again." } : item));
    setDownloads(checked); setStorage(await offlineStorageEstimate());
  }, []);
  ...
  return <main className="browse-shell"><TvSidebar /><header className="browse-header"><div><p className="eyebrow">TRAVEL LIBRARY</p><h1>Offline</h1><p>{online ? "Choose Download on any title before leaving home." : "Airplane mode ready — playing only from this device."}</p></div><span className={`connection-pill ${online ? "online" : "offline"}`}>{online ? "● Online" : "● Offline"}</span></header>
    ...
    {playing ? <section className="offline-player"><div className="offline-player-heading"><div><small>PLAYING FROM THIS DEVICE</small><h2>{playing.title}</h2></div><button className="secondary-button" onClick={() => { setPlaying(null); setPlaybackError(""); }}>Close</button></div><video key={playing.id} src={`/__offline/media/${playing.id}`} controls autoPlay playsInline preload="metadata" onError={() => setPlaybackError("The saved video could not be read. Remove it and download it again.")} />{playbackError ? <p className="offline-error">{playbackError}</p> : null}</section> : null}
    {!ready.length ? <section className="state-card"><strong>No verified downloads yet</strong><small>Open a movie or episode and select Download. It will stay inside Constant’s Hub rather than appearing in Files.</small><Link href="/tv" className="primary-button">Browse Library</Link></section> : <section className="offline-grid">{ready.map((item) => <article className="offline-card" key={item.id}>{item.poster ? <OfflinePoster blob={item.poster} title={item.title} /> : <div className="offline-poster-fallback">CH</div>}<div><small>{item.mediaType === "tv" ? `${item.seriesTitle || "TV Show"} · S${String(item.seasonNumber || 0).padStart(2, "0")}E${String(item.episodeNumber || 0).padStart(2, "0")}` : "MOVIE"}</small><h2>{item.title}</h2><p>{bytes(item.size)} · Verified offline</p><div className="hero-actions"><button className="primary-button" onClick={() => { setPlaybackError(""); setPlaying(item); }}>▶ Play</button><button className="secondary-button" onClick={() => void remove(item.id)}>Remove</button></div></div></article>)}</section>}
  </main>;
}
