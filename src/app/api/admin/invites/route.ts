...
import { clearInviteHistory, createInvite, deleteInvite, listInvites, revokeInvite } from "@/lib/app-data/invites";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await createInvite({ role: "family", expiresAt: typeof body.expiresAt === "string" && body.expiresAt ? body.expiresAt : null });
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (url.searchParams.get("history") === "true") return Response.json({ success: true, removed: await clearInviteHistory() });
    const id = url.searchParams.get("id") || "";
    const mode = url.searchParams.get("mode") || "revoke";
    if (mode === "delete") {
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Could not update invite." }, { status: 400 });
  }
}
