...
type TmdbResult = { id: number; title?: string; name?: string; release_date?: string; first_air_date?: string; overview?: string; poster_path?: string | null; vote_average?: number; genre_ids?: number[] };
type TmdbDetails = TmdbResult & { genres?: TmdbGenre[]; release_dates?: { results?: { iso_3166_1?: string; release_dates?: { certification?: string; type?: number }[] }[] } };
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (year) searchUrl.searchParams.set(type === "tv" ? "first_air_date_year" : "year", year);
    const headers = { Authorization: `Bearer ${token}`, accept: "application/json" };
    const [moviesResponse, genresResponse] = await Promise.all([
      fetch(searchUrl, { headers, signal: AbortSignal.timeout(8000) }),
      fetch(`https://api.themoviedb.org/3/genre/${type}/list?language=en-US`, { headers, signal: AbortSignal.timeout(8000) }),
    ]);
    if (!moviesResponse.ok || !genresResponse.ok) throw new Error("TMDB request failed.");
    const movies = await moviesResponse.json() as { results?: TmdbResult[] };
    const genres = await genresResponse.json() as { genres?: TmdbGenre[] };
    const movie = movies.results?.[0];
    if (!movie) return Response.json({ success: true, movie: null });
    const detailsResponse = await fetch(`https://api.themoviedb.org/3/${type}/${movie.id}?language=en-US${type === "movie" ? "&append_to_response=release_dates" : ""}`, { headers, signal: AbortSignal.timeout(8000) });
    ...
    const rating = type === "movie" ? certification(details) : null;
    return Response.json({ success: true, movie: {
      id: movie.id,
      title: type === "tv" ? movie.name : movie.title,
      year: (type === "tv" ? movie.first_air_date : movie.release_date)?.slice(0, 4) || null,
